...
export default function GalenaDynamicIsland() {
  const { currentStation, isPlaying, togglePlay, playNext, playPrev, auroraPalette, activeCategory, setActiveCategory } = useGalena();
  ...
  return (
    <div className={`flex lg:hidden mx-auto items-center justify-between rounded-[20px] bg-[#1c1c1e] dark:bg-[#0a0f1a] border border-white/10 dark:border-white/5 p-1 pr-2 transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] shadow-xl overflow-hidden ${
      isPlaying 
        ? 'max-w-[185px] sm:max-w-[210px] w-full opacity-100 scale-100' 
        : 'max-w-[100px] opacity-0 scale-95 pointer-events-none'
    }`}>
      ...
      <div className={`flex items-center gap-1 shrink-0 transition-all duration-300 ${isPlaying ? 'opacity-100' : 'opacity-0'}`}>
