...
export default function AnnouncementBoardWidget({ userId, userGroups, userRole = 'modelo' }: AnnouncementBoardWidgetProps) {
  ...
  return (
    <div className="flex flex-col gap-1.5 sm:gap-2 h-full">
      ...
      <div className="flex-1 glass-card bg-white/40 dark:bg-black/30 backdrop-blur-3xl border border-white/20 dark:border-white/5 max-sm:p-1.5 sm:p-2.5 rounded-[1.25rem] sm:rounded-2xl shadow-sm dark:shadow-none flex flex-col overflow-hidden">
            ...
            return (
              <div key={currentIndex} className="flex flex-col w-full h-full relative animate-in fade-in zoom-in-95 duration-500 ease-out">
                ...
                <div
                  ...
                  className="flex-1 relative group rounded-xl sm:rounded-xl md:rounded-2xl overflow-hidden cursor-pointer shadow-xl bg-white/50 dark:bg-[#1a1c29]/80 backdrop-blur-xl border border-white/50 dark:border-white/10 flex flex-col transition-all duration-300 hover:shadow-2xl hover:border-white/20"
                >
                  ...
                  <div className="relative w-full h-[70px] sm:h-28 lg:h-auto lg:flex-1 lg:min-h-[70px] bg-gray-100 dark:bg-black/40 overflow-hidden flex-shrink-0">
                    ...
                    {/* "Nuevo" Badge Absolute Top Right inside image container */}
                    {isNew && (
                      <span className="animate-pulse absolute top-4 right-4 inline-flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-bold text-white bg-gradient-to-r from-red-500 via-pink-500 to-red-500 rounded-full shadow-lg shadow-red-500/30 z-10 backdrop-blur-md">
                        <span>NUEVO</span>
                      </span>
                    )}
                  </div>
                  ...
                  <div className="relative z-10 px-3 py-2 sm:p-4 flex flex-col justify-between flex-1 min-h-0">
                    {/* Meta Top: Category Pill (Fixed height wrapper to prevent jumping) */}
                    <div className="h-[20px] sm:h-[22px] mb-1.5 sm:mb-2">
                      {currentItem.category && (
                        <span className="hidden sm:inline-block px-3 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider w-max"
                          style={{
                            backgroundColor: `${currentItem.category.color}15`,
                    ...
                    <div className="flex-1 flex flex-col justify-start mt-0">
                      {/* Headline (Fixed height to prevent 1-line vs 2-line jumps) */}
                      <h3 className="h-[44px] sm:h-[42px] text-[15px] sm:text-[16px] font-medium text-gray-800 dark:text-gray-100 leading-tight sm:leading-tight line-clamp-2 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors shrink-0">
                        {currentItem.title}
                      </h3>
                      
                      {/* Instructional Text */}
                      <p className="hidden sm:block text-[12px] sm:text-[13px] text-gray-500 dark:text-gray-400 leading-tight mt-1 sm:mt-1 line-clamp-1 shrink-0">
                        Ingresa a esta publicación para obtener la información completa.
                      </p>
                    </div>
                    ...
                    <div className="hidden sm:flex overflow-hidden mb-1.5 sm:mb-2 items-center gap-2">
                      {currentItem.author?.avatar_url ? (
                        ...
                        <div className="w-5 h-5 rounded-full bg-gray-200 dark:bg-gray-800 flex items-center justify-center text-[10px] text-gray-600 dark:text-gray-400 flex-shrink-0 border border-gray-300 dark:border-gray-700">
                          👤
                        </div>
                      )}
                      <span className="hidden sm:block text-[11.5px] text-gray-400 dark:text-gray-500 font-medium leading-none truncate">
                        Publicado por <span className="text-gray-600 dark:text-gray-300">{currentItem.author?.name || 'Administración'}</span>
                      </span>
                    </div>

                    {/* Meta Bottom: Date & Priority */}
                    <div className="flex items-center justify-between mt-auto shrink-0 pt-2 pb-0.5">
                      <div className="flex items-center gap-3">
                        <span className="text-sm font-medium text-gray-500 dark:text-gray-400">
                          {formatDate(currentItem.published_at || currentItem.created_at)}
                        </span>
                        {(currentItem.priority > 0 || currentItem.is_pinned) && (
                          <div className="hidden sm:flex items-center gap-1.5">
                            {currentItem.is_pinned && <span className="text-sm">📌</span>}
                            {currentItem.priority > 0 && (
                              <span className={`text-[10px] px-2.5 py-0.5 rounded font-extrabold uppercase tracking-widest ${
                                currentItem.priority === 2 ? 'bg-red-100 dark:bg-red-500/20 text-red-600 dark:text-red-400' : 'bg-orange-100 dark:bg-orange-500/20 text-orange-600 dark:text-orange-400'
                              }`}>
                                {currentItem.priority === 2 ? 'Urgente' : 'Alta'}
                              </span>
                            )}
                          </div>
                        )}
