  ...
  try {
    ...
    if (data.login_password_encrypted) {
      try {
        ...
        console.error('⚠️ Error desencriptando contraseña (clave de encriptación posiblemente incorrecta):', decryptError);
        // Devolver credenciales parciales en vez de un 500 — al menos el username es visible
        return NextResponse.json({
          success: true,
          hasCredentials: true,
