  ...
  return (
    <div className="min-h-screen relative w-full overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto max-sm:px-0 sm:px-6 lg:px-8 pb-4 sm:pb-2 pt-6 sm:pt-2 relative z-10">
        ...
        <div className="relative mb-6 sm:mb-8 overflow-hidden rounded-3xl bg-gradient-to-br from-pink-600 via-fuchsia-600 to-rose-500 shadow-xl border border-white/20 group">
          ...
          <div className="relative z-10 p-6 sm:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="flex items-center gap-4 sm:gap-5">
              <div className="flex-shrink-0 w-16 h-16 sm:w-20 sm:h-20 bg-white/20 backdrop-blur-xl rounded-[1.25rem] sm:rounded-[1.5rem] flex items-center justify-center shadow-[inset_0_0_15px_rgba(255,255,255,0.4)] border border-white/40 transform transition-all duration-500 hover:rotate-6 hover:scale-105">
                <svg className="w-8 h-8 sm:w-10 sm:h-10 text-white drop-shadow-[0_0_10px_rgba(255,255,255,0.5)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" />
                </svg>
              </div>
              <div>
                <h1 className="text-3xl sm:text-5xl font-black tracking-tighter mb-1 drop-shadow-md cursor-default">
            ...
            <div className="flex items-center gap-3 self-start md:self-center">
              {netoDisponible !== null && (
                <div className="hidden md:block bg-black/20 backdrop-blur-md rounded-2xl px-5 py-2.5 text-center border border-white/20 shadow-lg">
                  ...
                  <p className="text-xl font-bold text-white drop-shadow-sm">${netoDisponible.toLocaleString("es-CO")}</p>
                </div>
              )}
              {/* Cart button */}
              <button
                onClick={() => setShowCart(true)}
                className="relative bg-white/95 text-fuchsia-600 hover:bg-white rounded-2xl p-3.5 transition-all shadow-xl hover:shadow-2xl hover:-translate-y-0.5 active:scale-95 group"
              >
                <svg className="w-6 h-6 sm:w-7 sm:h-7 transition-transform group-hover:scale-110" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" />
                </svg>
                {cartCount > 0 && (
                  <span className="absolute -top-2 -right-2 w-6 h-6 bg-gradient-to-tr from-gray-900 to-black text-white text-xs font-bold rounded-full flex items-center justify-center shadow-lg border-2 border-white animate-bounce">
                    {cartCount}
                  </span>
                )}
              </button>
      ...
      {selectedProduct && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-gray-800 w-full md:max-w-4xl rounded-t-3xl sm:rounded-3xl shadow-2xl flex flex-col md:flex-row overflow-hidden max-h-[90vh]">
            
            {/* Zona de Imagen (Arriba en móvil, Izquierda en desktop) */}
            <div className="relative w-full md:w-1/2 min-h-[240px] md:min-h-full flex items-center justify-center bg-gradient-to-br from-pink-50 to-rose-100 dark:from-gray-700 dark:to-gray-600 shrink-0 p-6 md:p-10">
              {selectedProduct.images?.[0] ? (
                <img
                  src={selectedProduct.images[0]}
                  alt={selectedProduct.name}
                  className="max-w-full max-h-[35vh] md:max-h-[60vh] w-auto h-auto object-contain drop-shadow-xl"
                />
              ) : (
              ...
              <button
                ...
                className="md:hidden absolute top-4 right-4 w-8 h-8 bg-black/30 hover:bg-black/50 text-white rounded-full flex items-center justify-center transition-colors backdrop-blur-sm z-10"
              >
            ...
            <div className="w-full md:w-1/2 p-6 md:p-10 flex flex-col overflow-y-auto relative">
              ...
              <button
                onClick={() => setSelectedProduct(null)}
                className="hidden md:flex absolute top-5 right-5 w-8 h-8 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-500 rounded-full items-center justify-center transition-colors z-10"
              >
