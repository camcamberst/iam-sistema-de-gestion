          ...
          {user.role === 'modelo' && (
            <div className="order-1 lg:order-2 flex flex-col gap-1.5 sm:gap-2">
              ...
              <div className="flex items-center space-x-1 sm:space-x-1.5 px-1">
                ...
                <h2 className="text-[14px] sm:text-[15px] font-bold text-gray-900 dark:text-white tracking-tight drop-shadow-sm dark:drop-shadow-[0_0_8px_rgba(255,255,255,0.4)]">Productividad</h2>
              </div>
              ...
              {productivityLoading ? (
                ...
                <>
                  ...
                  <div className="glass-card bg-white/40 dark:bg-black/30 max-sm:dark:bg-black/60 backdrop-blur-3xl border border-white/20 dark:border-white/5 max-sm:dark:border-white/10 max-sm:p-1.5 sm:p-2 mb-1.5 sm:mb-2 rounded-[1.25rem] sm:rounded-2xl shadow-sm dark:shadow-none">
                    <InfoCardGrid
                      cards={[
                        {
                          value: productivityData ? `$${Math.round(productivityData.copModelo).toLocaleString('es-CO')}` : '—',
                          label: <><span className="sm:hidden">Ganancias</span><span className="hidden sm:inline">COP Modelo</span></>,
                          color: "purple",
                          size: "sm"
                        },
                        {
                          value: productivityData ? `$${productivityData.usdModelo.toFixed(2)}` : '—',
                          label: <><span className="sm:hidden">USD</span><span className="hidden sm:inline">USD Modelo</span></>,
                          color: "green",
                          size: "sm"
                        },
                        {
                          value: productivityData ? `$${productivityData.todayEarnings.toFixed(2)}` : '—',
                          label: <><span className="sm:hidden">Hoy</span><span className="hidden sm:inline">Ganancias Hoy</span></>,
                          color: "blue",
                          size: "sm"
                        }
                      ]}
