            ...
            {!loading && models.length > 0 && (
              <div className="space-y-2 sm:space-y-3">
                {Object.entries(grouped).map(([key, { groupName, models: gModels }]) => {
                  ...
                  const groupUsdModelo = gModels.reduce((s, m) => s + (m.usdModelo ?? 0), 0);
                  const groupUsdSede  = gModels.reduce((s, m) => s + (m.usdSede ?? 0), 0);
                  const groupCopModelo = gModels.reduce((s, m) => s + (m.copModelo ?? 0), 0);
                  const groupCopSede  = gModels.reduce((s, m) => s + (m.copSede ?? 0), 0);
                  ...
                  return (
                    <div key={key} className="rounded-xl border border-gray-200/60 dark:border-gray-600/40 overflow-hidden">
                      ...
                      >
                        <div className="flex items-center justify-between gap-2 px-3 sm:px-4 py-2.5 sm:py-3 bg-gray-50/80 dark:bg-gray-800/50 hover:bg-gray-100/70 dark:hover:bg-gray-700/50 transition-colors">
                        ...
                        {/* Consolidado: USD Modelo, USD Sede, COP Modelo, COP Sede — entre nombre de sede y barra */}
                                ...
                                <div key={model.modelId}
                                  ...
                                  <div className="hidden sm:flex items-center gap-3 mb-2">
                                    ...
                                    <span className="w-[118px] flex-shrink-0 text-xs tabular-nums text-right text-gray-600 dark:text-white">
                                      ${fmt(model.usdBruto)}<span className="text-gray-400 dark:text-white/70"> / ${fmt(model.cuotaMinima)}</span>
                                    </span>
                                  </div>
