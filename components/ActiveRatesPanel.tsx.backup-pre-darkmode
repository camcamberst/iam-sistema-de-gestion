...
export default function ActiveRatesPanel({ compact = false, showTitle = true, refreshTrigger, userRole }: ActiveRatesPanelProps) {
  ...
  if (compact) {
    return (
      <div className="relative bg-white/70 dark:bg-gray-700/70 backdrop-blur-sm rounded-xl shadow-md dark:shadow-[0_4px_12px_rgba(59,130,246,0.06)] dark:ring-1 dark:ring-blue-500/10 border border-white/20 dark:border-gray-600/20 p-3 sm:p-4 hover:shadow-xl hover:bg-white/95 dark:hover:bg-gray-600/80 hover:scale-[1.02] transition-all duration-300 cursor-pointer">
        {showTitle && (
          <div className="flex items-center space-x-2 mb-3">
            ...
            <div>
              <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Tasas de Calculadora</h3>
        ...
        {error ? (
          ...
          <div className="space-y-1">
            {rates.length === 0 ? (
              ...
              rates.map((rate) => (
                <div key={rate.id} className="flex items-center justify-between py-1 px-2 bg-gray-50 dark:bg-white rounded-md">
  ...
  return (
    <div className="bg-white/70 dark:bg-gray-700/70 backdrop-blur-sm rounded-xl p-3 sm:p-4 border border-white/20 dark:border-gray-600/20 shadow-md dark:shadow-[0_4px_12px_rgba(59,130,246,0.06)] dark:ring-1 dark:ring-blue-500/10">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center space-x-2">
          ...
          <div>
            <h2 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Tasas de Calculadora</h2>
      ...
      {error ? (
        ...
        <div className="space-y-2">
          {rates.length === 0 ? (
            ...
            rates.map((rate) => (
              <div key={rate.id} className="flex items-center justify-between px-3 py-2 bg-white dark:bg-white backdrop-blur-sm rounded-lg border border-gray-200/30 dark:border-gray-500/30 shadow-sm">
                <div className="flex items-center space-x-2">
                  <div className="w-6 h-6 bg-gray-50 dark:bg-gray-50 rounded-lg flex items-center justify-center">
                    <span className="text-xs">{getSourceIcon(rate.source)}</span>
                  </div>
                  <div>
                ...
                <div className="text-right">
                  <div className="text-sm font-bold text-gray-900 dark:text-gray-900">{rate.value}</div>
