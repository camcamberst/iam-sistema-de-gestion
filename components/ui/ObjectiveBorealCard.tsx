...
export default function ObjectiveBorealCard({
  ...
  if (isHistorical) {
    if (netoDisponibleCop !== undefined) {
      messages.push({
        text: `Neto final del periodo: $${Math.max(0, Math.round(netoDisponibleCop)).toLocaleString('es-CO')} COP`,
        color: "text-fuchsia-400 dark:text-fuchsia-400",
        bgClass: "bg-fuchsia-400",
        shadow: "drop-shadow-none dark:drop-shadow-[0_0_8px_rgba(232,121,249,0.5)]"
      });
    }
    
    if (estaPorDebajo) {
      messages.push({
        text: `Estuviste a tan solo: $${Math.ceil(cuotaMinima - totalUsdBruto)} USD (${remainingPct}%)`,
        color: "text-[rgba(74,188,150,0.9)] dark:text-cyan-400",
        bgClass: "bg-[rgba(74,188,150,0.9)] dark:bg-cyan-500",
        shadow: "drop-shadow-none dark:drop-shadow-[0_0_8px_rgba(34,211,238,0.5)]"
      });

      let motivationalTitle = "¡Este periodo fue de aprendizaje, el próximo la rompemos!";
      if (roundedProgress >= 75) motivationalTitle = "¡Estuviste muy cerca, a asegurar el objetivo el próximo periodo!";
      else if (roundedProgress >= 50) motivationalTitle = "¡Avanzamos bien, a darle con toda en el próximo!";
      
      messages.push({
        text: motivationalTitle,
        color: "text-indigo-400 dark:text-indigo-400",
        bgClass: "bg-indigo-400",
        shadow: "drop-shadow-none dark:drop-shadow-[0_0_8px_rgba(129,140,248,0.5)]"
      });
    } else {
      messages.push({
        text: `Resultado final: ¡Objetivo superado con +${Math.max(0, roundedProgress - 100)}%!`,
        color: "text-[rgba(74,188,150,0.9)] dark:text-emerald-400",
        bgClass: "bg-[rgba(74,188,150,0.9)] dark:bg-emerald-500",
        shadow: "drop-shadow-none dark:drop-shadow-[0_0_8px_rgba(52,211,153,0.5)]"
      });
      messages.push({
        text: "Desempeño final: ¡Objetivo consolidado y logrado exitosamente!",
    ...
    // Lógica para calculadora activa
    if (netoDisponibleCop !== undefined) {
      messages.push({
    ...
    if (anticipoMaxCop !== undefined) {
      messages.push({
        ...
        shadow: "drop-shadow-none dark:drop-shadow-[0_0_8px_rgba(167,139,250,0.5)]"
      });
    }
    if (estaPorDebajo && dailyAvg > 0) {
      messages.push({
        ...
        bgClass: "bg-blue-400",
        shadow: "drop-shadow-none dark:drop-shadow-[0_0_8px_rgba(96,165,250,0.5)]"
      });
    }
    if (estaPorDebajo) {
      messages.push({
    ...
    if (estaPorDebajo) {
      ...
      messages.push({
        text: motivationalTitle,
        color: "text-[rgba(74,188,150,0.9)] dark:text-emerald-300",
        bgClass: "bg-[rgba(74,188,150,0.9)] dark:bg-emerald-400",
        shadow: "drop-shadow-none dark:drop-shadow-[0_0_8px_rgba(52,211,153,0.5)]"
      });
    }
  }
  ...
  if (messages.length === 0) {
    messages.push({
      ...
      bgClass: "bg-gray-400",
      shadow: "drop-shadow-none dark:drop-shadow-[0_0_0_rgba(0,0,0,0)]"
    });
  }
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-[1.25rem] sm:rounded-2xl bg-black/[0.08] dark:bg-white/[0.04] backdrop-blur-3xl border border-white/40 dark:border-white/[0.08] max-sm:dark:border-white/[0.08] transition-all duration-700 shadow-sm shadow-black/5 dark:shadow-[0_1px_0_0_rgba(255,255,255,0.02)_inset,0_4px_20px_rgba(0,0,0,0.4)]`}
    >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-col sm:flex-row sm:items-center">
              <div className="flex items-center flex-1 min-w-0">
                ...
                <div className={`transition-all duration-500 ease-in-out w-[2.5px] h-3.5 sm:h-4 ml-1 mr-2 sm:mr-2.5 shrink-0 rounded-full ${currentMessage.bgClass} shadow-none dark:shadow-[0_0_6px_currentColor]`}></div>
                
                {/* Métrica Dinámica Marquee (Saliendo de la barra) */}
                <div
                  className={`text-xs sm:text-[13px] leading-none font-bold tracking-tight flex items-center h-5 flex-1 relative overflow-hidden`}
                >
                  <div 
                    key={msgIndex % messages.length}
                    className={`absolute inset-y-0 left-0 flex items-center whitespace-nowrap animate-slide-bar min-w-0 truncate ${currentMessage.color} message-glow max-sm:-translate-y-[0.5px]`}
                  >
                    {currentMessage.text}
                  </div>
                </div>
            ...
            <div className="mt-2.5 sm:mt-3 flex items-center gap-2 sm:gap-3">
              <div className="relative flex-1 bg-gray-900/80 rounded-full h-1.5 ring-1 ring-white/5">
                <div 
                  className="absolute left-0 top-0 h-full rounded-full transition-all duration-1000 ease-out shadow-[0_0_10px_current] flex items-center justify-end animate-gradient-x"
                  style={{ 
                    width: `${Math.min(porcentajeAlcanzado, 100)}%`,
                    backgroundImage: estaPorDebajo 
                      ? 'linear-gradient(90deg, #c026d3, #06b6d4, #10b981, #06b6d4, #c026d3)' 
