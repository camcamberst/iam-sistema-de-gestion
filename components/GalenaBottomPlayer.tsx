...
export default function GalenaBottomPlayer() {
  ...
  // En lugar de ocultar del DOM instantáneamente, mantenemos un isVisible suave para que el CSS actúe
  const isVisible = !isDashboard && (currentStation || isPlaying);
  ...
  return (
    <>
      <div className={`hidden lg:flex fixed bottom-6 left-1/2 -translate-x-1/2 z-[100] h-[76px] w-[90%] max-w-[900px] bg-[#0A0A0C]/80 dark:bg-[#030303]/80 backdrop-blur-3xl border border-white/10 rounded-[2.5rem] shadow-[0_20px_60px_rgba(0,0,0,0.6)] overflow-hidden items-center px-6 transition-all duration-[1200ms] ease-[cubic-bezier(0.25,0.8,0.25,1)] ${isVisible ? 'translate-y-0 opacity-100 scale-100 pointer-events-auto' : 'translate-y-[150%] opacity-0 scale-95 pointer-events-none'}`}>
        ...
        <div className="flex items-center w-[35%] min-w-[200px] h-full relative z-10">
           <div className={`relative w-14 h-14 rounded-full overflow-hidden shadow-[0_0_15px_rgba(0,0,0,0.5)] border-2 border-[#222] shrink-0 group transition-transform duration-1000 ${isPlaying ? 'animate-[spin_12s_linear_infinite]' : ''}`}>
            ...
            <div className="ml-4 flex items-center gap-3 min-w-0 pr-2">
               <span className="text-white text-[15px] font-bold tracking-tight truncate hover:text-[#22d3ee] transition-colors cursor-pointer drop-shadow-md">{currentStation?.name || 'Radio'}</span>
            </div>
         </div>
        ...
        <div className="flex-1 max-w-[30%] flex flex-col items-center justify-center relative z-10 h-full">
           <div className="flex items-center gap-6">
             ...
             <button onClick={stopAudio} className="text-white/40 hover:text-white transition-colors p-2 active:scale-90" title="Detener Frecuencia"><StopIcon /></button>
