...
export async function GET(req: NextRequest) {
  ...
  let query = supabase.from('shop_aisles').select('*').order('name');
...
export async function POST(req: NextRequest) {
  ...
  if (user.role !== 'super_admin' && user.role !== 'superadmin_aff') {
    return NextResponse.json({ error: 'Solo super admins pueden crear pasillos' }, { status: 403 });
  }
...
export async function PUT(req: NextRequest) {
  ...
  if (user.role !== 'super_admin' && user.role !== 'superadmin_aff') {
    return NextResponse.json({ error: 'Solo super admins pueden editar pasillos' }, { status: 403 });
  }
  ...
  // Verificar que el pasillo pertenece al mismo estudio
  ...
  const scope = getStudioScope(user);
  if (user.role !== 'super_admin' && aisle?.affiliate_studio_id !== scope) {
    return NextResponse.json({ error: 'No autorizado para este pasillo' }, { status: 403 });
  }
