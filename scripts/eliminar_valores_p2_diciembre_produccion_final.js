/**
 * 🗑️ SCRIPT: Eliminar Valores de P2 de Diciembre en Producción (FINAL)
 * 
 * Este script ejecuta la eliminación directamente usando service role key
...
if (!supabaseUrl || !supabaseKey) {
  ...
  console.error('   Estas deben ser las variables de PRODUCCIÓN');
  process.exit(1);
}
...
async function eliminarValoresProduccion() {
  console.log('\n🗑️ ELIMINACIÓN DE VALORES: P2 de Diciembre 2025 (PRODUCCIÓN)');
  ...
  try {
    ...
    if (residuales === 0) {
      ...
      // Mostrar modelos con valores residuales
      const modelosResiduales = Array.from(new Set(finalValores.map(v => v.model_id)));
      console.log(`   Modelos con valores residuales: ${modelosResiduales.length}`);
      modelosResiduales.slice(0, 10).forEach(modelId => {
        const email = emailMap.get(modelId) || modelId;
        console.log(`     - ${email}`);
      });
      if (modelosResiduales.length > 10) {
        console.log(`     ... y ${modelosResiduales.length - 10} más`);
      }
    }
